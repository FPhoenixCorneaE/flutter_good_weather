  ...
  Widget build(BuildContext context) {
    ...
    switch (direction) {
      case IntervalProgressDirection.horizontal:
        painter = HorizontalProgressPainter(max, progress, intervalSize, highlightColor, defaultColor, intervalColor,
            intervalHighlightColor, radius, reverse);
        break;
      case IntervalProgressDirection.vertical:
        painter = VerticalProgressPainter(max, progress, intervalSize, highlightColor, defaultColor, intervalColor,
            intervalHighlightColor, radius, reverse);
        break;
      case IntervalProgressDirection.circle:
        painter = CircleProgressPainter(max, progress, intervalSize, highlightColor, defaultColor, intervalColor,
            intervalHighlightColor, radius, reverse, intervalDegrees, strokeWith);
        break;
    }
...
abstract class IntervalProgressPainter extends CustomPainter {
  ...
  bool highlightBlock(int index) => reverse ? index >= (max - progress) : index < progress;

  bool highlightInterval(int index) => reverse ? index >= (max - progress - 1) : index < progress - 1;
  ...
  bool shouldDrawInterval(int index) =>
      index != max - 1 &&
      (intervalColor != IntervalProgressBar.TRANSPARENT || intervalHighlightColor != IntervalProgressBar.TRANSPARENT);
}

class HorizontalProgressPainter extends IntervalProgressPainter {
  HorizontalProgressPainter(int max, int progress, int intervalSize, Color highlightColor, Color defaultColor,
      Color intervalColor, Color intervalHighlightColor, double radius, bool reverse)
      : super(max, progress, intervalSize, highlightColor, defaultColor, intervalColor, intervalHighlightColor, radius,
            reverse, 0.0);

  @override
  Size calBlockSize() => Size(((bound.width - intervalSize * (this.max - 1)) / this.max), bound.height);

  @override
  void paintBlock(Canvas canvas, int i, Size? blockSize) {
    final blockWidth = blockSize?.width ?? 0;
    ...
    if (shouldDrawInterval(i)) {
      painter.color = highlightInterval(i) ? intervalHighlightColor : intervalColor;
      canvas.drawRect(
          Rect.fromLTRB(
            blockWidth,
            0,
  ...
  Rect _drawLeftRound(Canvas canvas, Rect rect) {
    final clipRect = Rect.fromLTRB(rect.left + radius, rect.top, rect.right, rect.bottom);
    _drawRadius(canvas, rect, clipRect);
    return clipRect;
  }

  Rect _drawRightRound(Canvas canvas, Rect rect) {
    final clipRect = Rect.fromLTRB(rect.left, rect.top, rect.right - radius, rect.bottom);
    _drawRadius(canvas, rect, clipRect);
    return clipRect;
  }

  void _drawRadius(Canvas canvas, Rect rect, Rect clipRect) {
    final roundRect = RRect.fromLTRBR(rect.left, rect.top, rect.right, rect.bottom, Radius.circular(radius));
...
class VerticalProgressPainter extends IntervalProgressPainter {
  VerticalProgressPainter(int max, int progress, int intervalSize, Color highlightColor, Color defaultColor,
      Color intervalColor, Color intervalHighlightColor, double radius, bool reverse)
      : super(max, progress, intervalSize, highlightColor, defaultColor, intervalColor, intervalHighlightColor, radius,
            reverse, 0.0);
  ...
  @override
  void paintBlock(Canvas canvas, int i, Size? blockSize) {
    final blockHeight = blockSize?.height ?? 0;
    ...
    if (shouldDrawInterval(i)) {
      painter.color = highlightInterval(i) ? intervalHighlightColor : intervalColor;
      final intervalRect = Rect.fromLTRB(0, blockHeight, bound.width, blockHeight + intervalSize);
  ...
  @override
  Size calBlockSize() => Size(bound.width, (bound.height - intervalSize * (this.max - 1)) / this.max);

  Rect _drawStartRadius(Canvas canvas, Rect rect) {
    final clipRect = Rect.fromLTRB(rect.left, rect.top + radius, rect.right, rect.bottom);
    _drawRadius(canvas, rect, clipRect);
    return clipRect;
  }

  Rect _drawEndRadius(Canvas canvas, Rect rect) {
    final clipRect = Rect.fromLTRB(rect.left, rect.top, rect.right, rect.bottom - radius);
    _drawRadius(canvas, rect, clipRect);
    return clipRect;
  }

  void _drawRadius(Canvas canvas, Rect rect, Rect clipRect) {
    final rRect = RRect.fromLTRBR(rect.left, rect.top, rect.right, rect.bottom, Radius.circular(radius));
...
class CircleProgressPainter extends IntervalProgressPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final incr = (360.0 - intervalDegrees * (this.max - 1)) / (this.max);
    final intervalRadians = degrees2radians(intervalDegrees);
